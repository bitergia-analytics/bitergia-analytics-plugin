...
import { IBasePath, IRouter } from '../../../../../src/core/server';
import { getRolesMapping } from './roles_mapping';

export function registerCreateTenantRoute(
  router: IRouter,
  basePath: IBasePath
) {
  router.post(
    {
      path: '/api/create_tenant',
      validate: {
        body: schema.object({
    ...
    async (context, request, response) => {
      const { tenant, anonymous, force } = request.body;
      const config = {
        headers: {
...
async function mapRoles(
  ...
  for (const role of roles) {
    ...
    } else if (role.includes('plugins_visibility')) {
      user = 'hide_plugins';
    } else {
      continue;
    }
